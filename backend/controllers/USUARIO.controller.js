...
export const createUsuario = async (req, res) => {
  try {
    const { usuario, id_funcionario, contraseña, rol } = req.body;
    
    // Validación de campos obligatorios
    if (!usuario || !id_funcionario || !contraseña || !rol) {
      return res.status(400).json({ 
        message: "Faltan campos requeridos: usuario, id_funcionario, contraseña, rol" 
      });
    }
