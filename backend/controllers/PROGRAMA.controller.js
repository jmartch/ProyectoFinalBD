...
export const createPrograma = async (req, res) => {
  try {
    ...
    //Validación de longitud de id_programa
    if (id_programa.length > 10) {
      return res.status(400).json({ 
        message: "El ID del programa no puede exceder 10 caracteres" 
      });
    }
    ...
    try {
      ...
      const result = await Programa.remove(id_programa);
        ...
        if (error.code === "ER_ROW_IS_REFERENCED_2") {
            return res.status(409).json({
                message: "No se puede eliminar el programa porque está asociado a otros registros"
            });
        }
