  ...
  try {
    ...
    if (!id_periodo || !nombre || porcentaje === undefined) {
      return res.status(400).json({
        message:
          "Faltan campos requeridos: id_periodo, nombre, porcentaje",
      });
    }
    ...
    const result = await COMPONENTE.create({
      id_periodo,
      nombre,
      porcentaje,
    });
...
export const updateComponente = async (req, res) => {
  try {
    ...
    const result = await COMPONENTE.update(id, {
      id_periodo,
      nombre,
      porcentaje,
    });
  ...
  try {
    ...
    if (error.code === "ER_ROW_IS_REFERENCED_2") {
      return res.status(409).json({
        message:
          "No se puede eliminar el componente porque tiene registros asociados",
      });
    }
    res.status(500).json({
