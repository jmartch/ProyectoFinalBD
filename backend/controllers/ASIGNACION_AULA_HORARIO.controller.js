...
export const getAsignacionByKeys = async (req, res) => {
  try {
    ...
    const asignacion = await Asignacion_aula_horario.getByKeys(
      id_horario,
      id_aula,
      fecha_inicio
    );
  ...
  try {
    ...
    if (!id_horario || !id_aula || !fecha_inicio) {
      return res.status(400).json({
        message:
          "Faltan campos requeridos: id_horario, id_aula, fecha_inicio",
      });
    }
    ...
    if (!fechaRegex.test(fecha_inicio)) {
      return res.status(400).json({
        message:
          "Formato de fecha_inicio inválido. Use YYYY-MM-DD",
      });
    }

    // Validar formato de fecha_fin solo si viene
    if (fecha_fin && !fechaRegex.test(fecha_fin)) {
      return res.status(400).json({
        message:
          "Formato de fecha_fin inválido. Use YYYY-MM-DD",
      });
    }

    // Validar que la fecha de asignación no sea futura (si esa es la regla de negocio)
    ...
    // Manejo de asignación duplicada (PK compuesta)
    if (error.code === "ER_DUP_ENTRY") {
      return res.status(409).json({
        message:
          "Ya existe una asignación para este horario en este aula con esa fecha de inicio",
      });
    }
...
export const updateAsignacionByKeys = async (req, res) => {
  try {
    ...
    // Validar que la fecha no sea futura (si mantienes esta regla)
    ...
    const result = await Asignacion_aula_horario.updateByKeys(
      id_horario,
      id_aula,
      fecha_inicio,
      { fecha_fin }
...
export const deleteAsignacionByKeys = async (req, res) => {
  try {
    const { id_horario, id_aula, fecha_inicio } = req.params;
    const result = await Asignacion_aula_horario.removeByKeys(
      id_horario,
      id_aula,
      fecha_inicio
    );
    ...
    if (error.code === "ER_ROW_IS_REFERENCED_2") {
      return res.status(409).json({
        message:
          "No se puede eliminar la asignación porque tiene registros asociados",
      });
    }
    res.status(500).json({
