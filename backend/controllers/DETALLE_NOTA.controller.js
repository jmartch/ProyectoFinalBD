  ...
  try {
    ...
    if (!id_nota || !id_componente || nota === undefined) {
      return res.status(400).json({ 
        message: "Faltan campos requeridos: id_nota, id_componente, nota" 
      });
    }

    // Validación de nota (asumiendo escala de 0 a 5 o 0 a 100, ajusta según tu sistema)
    if (nota < 0 || nota > 5) {
      return res.status(400).json({ 
    ...
    const result = await DETALLE_NOTA.create({ id_nota, id_componente, nota });
    res.status(201).json({ 
    ...
    if (error.code === 'ER_DUP_ENTRY') {
      return res.status(409).json({ 
        message: "Ya existe un detalle de nota para este componente en esta nota" 
      });
    }
...
export const updateDetalleNota = async (req, res) => {
  try {
    ...
    const result = await DETALLE_NOTA.updateByKeys(id_nota, id_componente, { nota });
...
export const deleteDetalleNota = async (req, res) => {
  try {
    const { id_nota, id_componente } = req.params;
    const result = await DETALLE_NOTA.removeByKeys(id_nota, id_componente);
