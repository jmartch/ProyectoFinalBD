  ...
  try {
    ...
    if (!dia_semana || !hora_inicio || !horas_duracion) {
      return res.status(400).json({
        message:
          "Faltan campos requeridos: dia_semana, hora_inicio, horas_duracion",
      });
    }
    ...
    if (!diasValidos.includes(dia_semana)) {
      return res.status(400).json({
        message:
          "Día de la semana inválido. Valores permitidos: Lunes, Martes, Miércoles, Jueves, Viernes, Sábado, Domingo",
      });
    }
    ...
    if (!horaRegex.test(hora_inicio)) {
      return res.status(400).json({
        message:
          "Formato de hora inválido. Use HH:MM:SS (ejemplo: 08:00:00)",
      });
    }
    ...
    if (isNaN(horas_duracion) || horas_duracion <= 0 || horas_duracion > 24) {
      return res.status(400).json({
        message:
          "Horas de duración inválidas. Debe ser un número entre 1 y 24",
      });
    }
...
export const updateHorario = async (req, res) => {
  try {
    ...
    // Validar que haya datos completos para actualizar (update completo)
    if (!dia_semana || !hora_inicio || horas_duracion === undefined) {
      return res.status(400).json({
        message:
          "Faltan campos requeridos para actualizar: dia_semana, hora_inicio, horas_duracion",
      });
    }
    ...
    // Validación de horas de duración
    if (isNaN(horas_duracion) || horas_duracion <= 0 || horas_duracion > 24) {
      return res.status(400).json({
        message:
          "Horas de duración inválidas. Debe ser un número entre 1 y 24",
    ...
    const result = await Horario.update(id, {
      dia_semana,
      hora_inicio,
      horas_duracion,
    });
    ...
    res.json({
      ...
      data: { id_horario: id, dia_semana, hora_inicio, horas_duracion },
    });
  } catch (error) {
    res.status(500).json({
