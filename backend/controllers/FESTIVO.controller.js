  ...
  try {
    ...
    if (!fecha || !descripcion) {
      return res.status(400).json({
        message: "Faltan campos requeridos: fecha, descripcion",
      });
    }
...
export const updateFestivo = async (req, res) => {
  try {
    const { id } = req.params;
    const { descripcion, fecha } = req.body;
    ...
    const result = await Festivo.updateById(id, { descripcion, fecha });
...
export const deleteFestivo = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await Festivo.removeById(id);
    ...
    if (error.code === "ER_ROW_IS_REFERENCED_2") {
      return res.status(409).json({
        message:
          "No se puede eliminar el festivo porque tiene registros asociados",
      });
    }
    res.status(500).json({
