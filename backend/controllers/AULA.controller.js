...
export const createAula = async (req, res) => {
  try {
    const { id_sede, grado, programa } = req.body;
    
    // Validación básica
    if (!id_sede || !grado || !programa) {
      return res.status(400).json({ 
        message: "Faltan campos requeridos: id_sede, grado, programa" 
      });
    }
    
    const result = await AULA.create({ id_sede, grado, programa });
    res.status(201).json({ 
...
export const updateAula = async (req, res) => {
  try {
    ...
    const result = await AULA.update(id, { id_sede, grado, programa });
  ...
  try {
    ...
    if (error.code === 'ER_ROW_IS_REFERENCED_2') {
      return res.status(409).json({ 
        message: "No se puede eliminar el aula porque tiene registros asociados" 
      });
    }
    res.status(500).json({ 
